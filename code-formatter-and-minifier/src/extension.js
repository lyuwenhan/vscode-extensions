...
function activate (context) {
	...
	const flipCoinCmds = [vscode.commands.registerCommand("minifier.flipCoin", async () => {
		try {
			const {
				result,
				current
			...
			if (result) {
				vscode.window.showInformationMessage(`Try #${current.heads+current.tails}: Heads.`)
			} else {
				vscode.window.showWarningMessage(`Try #${current.heads+current.tails}: Tails.`)
			}
		} catch (e) {
			vscode.window.showErrorMessage(e.message || String(e))
		}
		...
		try {
			...
			vscode.window.showInformationMessage(`Today's Coin Stats: ${stats.heads} Heads and ${stats.tails} Tails.\nHeads rate: ${Math.round(100*stats.heads/(stats.heads+stats.tails))}%`)
		} catch (e) {
			vscode.window.showErrorMessage(e.message || String(e))
		}
	...
	context.subscriptions.push(generateUuidCmd, ...flipCoinCmds)
}
